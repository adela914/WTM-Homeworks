  ...
  v-row(justify="center")
    v-dialog(v-model="dialog" persistent max-width="600px")
      template(v-slot:activator="{ on }")
        v-btn(color="rgb(253, 210, 74)" dark v-on="on") Edit
      v-card
        v-card-title
            ...
            v-container
              v-row
                v-col(cols="12" sm="4" md="6" )
                  v-text-field(label="Restaurant's name" v-model="restaurant.name")
                v-col(cols="12" sm="4" md="6")
                  v-text-field(label="Location" v-model="restaurant.location")
